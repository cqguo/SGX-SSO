...
%
\begin{abstract}
Single sign-on (SSO) services are widely deployed on the Internet as the identity management and authentication infrastructure. In an SSO system, after authenticated by the identity providers (IdPs), a user is allowed to log into relying parties (RPs) by submitting an \emph{identity proof}.
However, SSO introduces the potential leakage of user privacy, which is indicated by NIST.  That is (\emph{a}) a curious IdP could track a user's all visits to any RPs and (\emph{b}) collusive RPs could link the user's identities across different RPs, to learn the user's activity profile.
