...
\begin{breakablealgorithm}
  ...
  \SWITCH{\myss{s'.q}}
    ...
    \CASE{expectUIDToken}
      ...
      \mystate{\myss{command := \langle \mathtt{ENCLAVEMESSAGE}, \langle \myangle{\mathtt{UIDToken}, token},}}
      \Statex \myss{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \myangle{\mathtt{RPDomain},s'.Parameters[RPDomain]}\rangle \rangle}
...
The state of enclave application is a term in the form \\ \myss{\myangle{IdP, Parameters}}.
\vspace{-3mm}
\begin{itemize}
\setlength\itemsep{-2pt}
\item \myss{IdP} contains the IdP's information, including $Host$, $path$, and so on.
\item \myss{Parameters} is used to store the parameters received from other processes.
\end{itemize}
\vspace{-3mm}
\begin{breakablealgorithm}
